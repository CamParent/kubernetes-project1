...
- **Automated CI/CD**: a self-hosted GitHub Actions pipeline builds, Trivy-scans, and pushes the image on every commit, gated so a CRITICAL/HIGH vulnerability blocks the push entirely
...
CI/CD: push to main -> self-hosted runner -> build -> Trivy scan (blocks on CRITICAL/HIGH) -> push to GHCR (commit SHA + latest tags)
...
- `.github/workflows/` — CI/CD pipeline definition (build, scan, push)
...
- `docs/cicd-pipeline.md` — full writeup of the CI/CD phase: pipeline design, self-hosted runner setup, and debugging notes
